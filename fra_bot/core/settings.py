...
SETTINGS: tuple[Setting, ...] = (
    ...
    Setting("discord.channels.vehicles_forum", "int", True,
            "Vehicles-database forum channel (0 = off)"),
    Setting("discord.channels.vehicle_announce", "int", True,
            "New-vehicle announcement channel (0 = off)"),
    Setting("discord.channels.dm_mirror", "int", True,
    ...
    Setting("discord.channels.chat_bridge", "int", True,
    ...
    Setting("discord.channels.mc_status", "int", True,
            "MissionChief outage/recovery notices for members (0 = off)"),
    ...
    Setting("discord.system_message_role_id", "int", True,
            "Role pinged above system-message embeds (0 = no ping)"),
    Setting("discord.event_watch_channel_id", "int", True,
            "Channel watched for MissionChief app announcements (0 = off)",
            aliases=("event_watch_channel",)),
    Setting("discord.event_watch_app_id", "int", True,
            "The MissionChief app's user id whose announcements are pinged"),
    ...
    Setting("automation.training.enabled", "bool", True, "Training auto-start"),
    ...
    Setting("automation.academy.enabled", "bool", True,
            "Academy autonomous spending (extension sweep; queue drain)"),
    Setting("automation.academy.role_id", "int", True,
            "Role allowed to click the academy panel (0 = admins only)",
            aliases=("academy_role",)),
    Setting("automation.academy.interval", "int", False,
            "Academy queue retry interval (min)", minimum=2),
    Setting("automation.academy.address", "str", True,
    ...
    Setting("automation.vehicles_forum.enabled", "bool", False,
            "Daily vehicles-forum sync"),
    Setting("automation.vehicles_forum.sync_time", "time", False,
            "Vehicles-forum sync time (HH:MM, reports timezone)"),
    Setting("automation.vehicles_forum.announce_new", "bool", True,
            "Ping the announcement channel for new vehicles"),
    Setting("automation.vehicles_forum.max_posts_per_run", "int", True,
            "Vehicles-forum posts/edits per sync run", minimum=1, maximum=500),
    Setting("automation.dm_mirror.enabled", "bool", False,
            "In-game DM inbox scan (mirror to the forum)"),
    Setting("automation.dm_mirror.interval", "int", False,
            "Minutes between DM inbox scans", minimum=2, maximum=120),
    Setting("automation.rank_roles.enabled", "bool", False,
            "Credit rank roles (hourly sync)"),
    Setting("automation.rank_roles.interval", "int", False,
            "Minutes between rank-role syncs", minimum=15, maximum=1440),
    Setting("automation.rank_roles.promotion_channel_id", "int", True,
            "Channel for promotion announcements",
            aliases=("promotion_channel",)),
    Setting("automation.rank_roles.announce_first_assignment", "bool", True,
            "Also congratulate a member's FIRST rank assignment"),
    ...
    Setting("automation.chat.enabled", "bool", True,
            "Alliance chat ↔ Discord bridge"),
    Setting("automation.welcome.enabled", "bool", True,
            "Welcome new members in the game chat"),
    Setting("automation.welcome.message", "str", True,
            "New-member welcome text ({name} = member's game name)"),
    Setting("automation.chat.interval_seconds", "int", False,
            "Chat bridge poll interval (seconds)", minimum=30, maximum=3600),
    Setting("automation.mc_status.enabled", "bool", True,
            "Announce MissionChief outages/recoveries to members"),
    Setting("automation.mc_status.outage_minutes", "int", True,
            "Minutes unreachable before the outage notice", minimum=1,
            maximum=720),
    Setting("automation.sanctions.game_log_review_enabled", "bool", True,
            "Import game-log kicks/chat bans as unverified sanctions"),
    Setting("automation.sanctions.escalation_mode", "str", True,
            "CoC-5 escalation: advisory text, admin buttons, or automatic",
            choices=("advisory", "button", "auto")),
    Setting("automation.sanctions.escalation_offense_threshold", "int", True,
            "Offense count that triggers the CoC 5.3 removal step",
            minimum=2, maximum=10),
    Setting("automation.sanctions.escalation_gap_hours", "int", True,
            "Auto mode: hours after the newest offense before acting",
            minimum=1, maximum=168),
    Setting("automation.sanctions.escalation_notice", "bool", True,
            "PM the member before an escalation kick"),
    Setting("automation.sanctions.escalation_mute_type", "str", True,
            "Mute issued at the CoC 5.2 escalation step",
            choices=("Mute 5m", "Mute 15m", "Mute 30m", "Mute 1h", "Mute 6h",
                     "Mute 12h", "Mute 1d", "Mute 7d", "Mute 14d")),
    Setting("automation.sanctions.mute_execution_enabled", "bool", True,
            "Mute sanctions really set the in-game chat ban "
            "(only enable once the chat-ban route is verified)"),
    Setting("automation.sanctions.reapply_block_days", "int", True,
            "CoC 5.3: days a kicked/banned member must wait before returning",
    ...
    Setting("automation.mission.enabled", "bool", True, "Mission queue scheduler"),
    Setting("automation.mission.board_enabled", "bool", True, "Mission board polling"),
