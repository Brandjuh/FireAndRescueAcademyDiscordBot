...
async def test_training_guide_creates_all_sections_then_skips(db):
    ...
    assert "short [code]" in board.created[0][1]   # overview names the codes
    fire_post = board.created[1][1]
    assert fire_post.startswith("[FRA] 📋 Fire Station training request text")
    assert "- [HAZ] HazMat (3 days)" in fire_post
    assert ("- [LGTF] Fire Station - Lifeguard Training (5 days) "
            "- opens Lifeguard Training") in fire_post
    ...
    # the bare name.
    assert "- [EMCF] Fire Station - EMS Mobile Command (7 days)" in fire_post
    ems_post = board.created[3][1]
    assert ems_post.startswith("[FRA] 📋 EMS training request text")
    assert "- [EMC] EMS Mobile Command (7 days)" in ems_post
